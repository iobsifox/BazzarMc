...
> **از نسخهٔ ۱.۸.۲:** هر درخواستی که `auth()` آن را تأیید کند (یا رد کند) در گزینهٔ `bmc_api_activity` ثبت می‌شود — زمان، آی‌پی، مسیر و تعداد موفق‌ها به‌همراه آخرین تلاش ناموفق و دلیل آن (`invalid_token` / `no_token`). پنل وضعیت «اتصال سرور بازی» را از همین داده می‌خواند: فهرست راه‌اندازی پیشخوان، تب *اتصال سرور* و بررسی سلامت. ثبت **حداکثر یک بار در دقیقه** انجام می‌شود. توکن درست در `config.yml` + یک `bmc ping` در کنسول، برای سبز شدن مرحله کافی است.
...
| `channel` | string | – | روش تحویل کد: `email` (پیش‌فرض از ۱.۸.۱ — اگر خالی/نفرستاده باشد هم `email` در نظر گرفته می‌شود) یا `dashboard` (کد در پاسخ برمی‌گردد و در پنل کاربری و ویجت پیشخوان نمایش داده می‌شود) |
...
**بازگشت خودکار به داشبورد (جدید در ۱.۸.۱):** اگر ارسال ایمیل به هر دلیلی ناموفق باشد (SMTP خاموش، `wp_mail` = false، خطای هاست) ولی کانال **داشبورد** روشن باشد و کاربر پیدا شده باشد، درخواست **ناموفق نمی‌شود**؛ کد ذخیره می‌شود و پاسخ موفق با `channel: "dashboard"`، `fallback: true` و خودِ `code` برمی‌گردد تا بازیکن همان لحظه در چت کد را ببیند و با `/mclink verify <code>` حساب را متصل کند. دلیل واقعی شکست ایمیل هم در لاگ افزونه ثبت می‌شود.

```json
{
  "success": true,
  "message": "ارسال ایمیل ممکن نشد؛ کد تأیید در داشبورد حساب کاربری شما نمایش داده شد.",
  "channel": "dashboard",
  "code": "48213",
  "expires_in": 300,
  "fallback": true,
  "reason": "wp_mail() مقدار false برگرداند"
}
```

> پلاگین ماینکرافت از ۱.۸.۱ اگر در پاسخ `code` ببیند، آن را با کلیدهای `messages.otp-code-inline` و `messages.otp-code-command` در چت چاپ می‌کند؛ در غیر این صورت همان پیام «کد ارسال شد» را نشان می‌دهد.

خطاها: `DISABLED` (روش غیرفعال)، `RATE` (محدودیت نرخ)، `FORMAT` (ایمیل نامعتبر یا حساب یافت نشد)، `SEND_FAIL` (ارسال ایمیل ناموفق بود **و** کانال داشبورد هم در دسترس نیست)، `CHANNEL` (هیچ روش تأییدی در دسترس نیست)، `NO_USER`، `ALREADY_LINKED`، `PLAYER_TAKEN`.
